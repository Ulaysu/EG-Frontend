...
async function handleFiles(files) {
  ...
  // UPLOAD IMMEDIATELY
  try {
    isSubmitting.value = true // optional UI lock during upload

    const url = await uploadImage(file)
    imageUrl.value = url
  } catch (err) {
    errors.image = 'Image upload failed. Try again.'
  } finally {
...
function changeImage() { openFilePicker() }

// ---------- Validation ----------
function validate() {
  Object.keys(errors).forEach((k) => delete errors[k])
  if (!form.title.trim()) errors.title = 'Tour title is required.'
  else if (form.title.length < 4) errors.title = 'Title must be at least 4 characters.'
  if (!form.description.trim()) errors.description = 'Description is required.'
  else if (form.description.length < 20) errors.description = 'Please write at least 20 characters.'
  ...
  if (!form.maxParticipants || Number(form.maxParticipants) < 1)
    errors.maxParticipants = 'At least 1 participant.'
  if (!form.startDate) errors.startDate = 'Start date is required.'
  if (!form.endDate) errors.endDate = 'End date is required.'
  if (form.startDate && form.endDate && form.endDate < form.startDate)
    errors.endDate = 'End date must be after start date.'
if (!imageUrl.value) errors.image = 'Please wait for image to finish uploading.'
  if (!imageFile.value) errors.image = 'Please upload a cover image.'
...
async function onSubmit() {
  ...
  try {
    const payload = {
      ...
      imageUrl: imageUrl.value // We upload the image immediately on selection, so we should have the URL ready by now.
    }
...
imagePreview.value = ''
    ...
    isSubmitting.value = false
    submitError.value = error.message || 'Failed to create tour.'
  }
}
  ...
  <div class="et-page">
    ...
    <transition name="et-toast">
      <div v-if="showSuccess" class="et-toast" role="status">
        ...
        <span>Tour created successfully!</span>
      </div>
    </transition>
    ...
    <div class="et-container">
      ...
      <header class="et-header">
        ...
        <h1 class="et-title">Create a New Tour</h1>
        <p class="et-subtitle">Share your experience with travelers from around the world.</p>
      </header>
